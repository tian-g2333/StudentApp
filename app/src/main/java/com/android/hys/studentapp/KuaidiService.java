...
public class KuaidiService {
    public static boolean kuaidiSelect(String name, String info) {
        KuaidiThread kuaidiThread = new KuaidiThread("http://8.129.48.153:8080/MyWeb/SignIn",name,info);
        try
        {
            kuaidiThread.start();
            kuaidiThread.join();
    ...
    public static boolean kuaidiAdd(String name, String info) {
        KuaidiThread kuaidiThread = new KuaidiThread("http://8.129.48.153:8080/MyWeb/DeliveryAdd",name,info);
        try
        {
            kuaidiThread.start();
            kuaidiThread.join();
...
class KuaidiThread extends Thread
{
    ...
    public KuaidiThread(String path,String name,String info)
    {
        this.path = path;
        this.name = name;
        this.info = info;
    ...
    public void run()
    {
        try {
            ...
            String data = "name=" + URLEncoder.encode(name, "utf-8") + "&info=" + URLEncoder.encode(info, "utf-8");//设置数据
